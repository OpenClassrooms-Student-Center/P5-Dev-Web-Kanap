        ...
        .then((data) => {
          ...
          for (let quantity of productQuantity) {
            quantity.addEventListener("change", function (e) {
              ...
              console.log(product_id, product_color);
      ...
      .then((data) => {
        ...
        for (let quantity of productQuantity) {
          quantity.addEventListener("change", function (e) {
            ...
            const product = panier.map((element) => {
              if (element.id == product_id && element.color == product_color) {
                element.quantity = qte;
              }
              return element;
            });
...
if (panier != null) {
  const getPanierId = () => {
    let itemsId = [];
    for (let itemId of panier) {
      itemsId.push(itemId.id);
    }
    return itemsId;
  };
  getPanierId();
  let productID = getPanierId();
...
const validForm = (userInput, regExp, elt) => {
  ...
  if (!regExp.test(userInput) || userInput == "") {
    elt.nextElementSibling.textContent = "Veuillez respeter le format";
    elt.setAttribute(
      "placeholder",
      `Veuillez entre votre ${txt.toLowerCase().replace(":", "")}`
    );
    elt.style.color = "red";
    return false;
  }
